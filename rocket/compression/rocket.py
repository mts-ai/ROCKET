...
import gc
from rocket.utils.model_utils import get_weight_transposed, get_k_and_sparsity, frobenius_distance
...
def svd_with_magnitude_sparsity_on_v(
    ...
    with torch.no_grad():
        ...
        u = eigvecs[:, :r]      # (d1, r)
        Lambda_r = eigvals[:r]    # (r,)
        Lambda_r = torch.clamp(Lambda_r, min=0.0)
        v_full = u.t() @ x 
    ...
    del w1, ss, inv_s, u, v_full, x, G, G_reg, WVt
    gc.collect()
