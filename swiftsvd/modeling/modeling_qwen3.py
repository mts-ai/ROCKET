...
class Qwen3MLP_SVD(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.gate_proj_v = nn.Linear(self.gate_low_rank, self.intermediate_size, bias=False)
        self.up_proj_u = nn.Linear(self.hidden_size, self.up_low_rank, bias=False)
        self.up_proj_v = nn.Linear(self.up_low_rank, self.intermediate_size, bias=False)
        self.down_proj_u = nn.Linear(self.intermediate_size, self.down_low_rank, bias=False)
        self.down_proj_v = nn.Linear(self.down_low_rank, self.hidden_size, bias=False)
        self.act_fn = ACT2FN[config.hidden_act]
